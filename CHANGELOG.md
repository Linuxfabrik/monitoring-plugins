...
* mysql-aria: the absence or explicit deactivation of the Aria storage engine no longer triggers UNKNOWN. The plugin now reports OK with an info message in those cases (vanilla MySQL has no Aria, and disabling it is a config decision, not an unknown state). DESCRIPTION corrected to reflect that only the page-cache hit rate is tracked, not separate read and write rates. Ships a dedicated Grafana dashboard with three panels splitting the metrics by magnitude (sizes, read activity counters, hit-rate percentage) so the smaller counters no longer disappear next to the buffer-size series
