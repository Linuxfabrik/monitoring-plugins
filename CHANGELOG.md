...
* about-me: `--tags` now correctly distinguishes MariaDB from MySQL
...
* mysql-database-metrics: new `--ignore-schemas REGEX` and `--lengthy`. Director Basket defaults `--lengthy=true`, pre-fills `--ignore-schemas=^icinga`
* mysql-database-metrics: excludes `percona` schema (was falsely flagged). Index-vs-data-size check fixed. New `--ignore-tables REGEX`. Now emits perfdata. Ships Grafana dashboard
* mysql-innodb-buffer-pool-size!: workload-based `innodb_redo_log_capacity` check on MySQL 8.0.30+. New `innodb_file_per_table` check. Breaking perfdata. Ships Grafana dashboard
...
* mysql-system: new WARN on `fs.nr_open < 1M`. Listening-port count limited to LISTEN state (was over-triggering). Breaking perfdata: `kernel.*` labels and `mysql_opened_ports` renamed
* mysql-table-cache!: new `--warning` (20%) / `--critical` (10%). Breaking perfdata: cumulative counters replaced by per-second rates
...
* about-me: Non-default Software, Non-default Users and systemctl list-timers tables now sort case-insensitive and natural (so `foo10` lands after `foo2`)
...
* users: cross-distro container-based test added, covering RHEL 10 SSH sessions without a PTY ([#989](https://github.com/Linuxfabrik/monitoring-plugins/issues/989))
...
* dmesg: `--ignore` now takes a regex (was substring) and is repeatable. Specifying it replaces the bundled defaults (was extending). Defaults grew to cover SHPC PCI hot-plug noise
* dmesg: `--severity` deprecated; the plugin always alerts as CRIT. Existing templates with `dmesg_severity = warn` keep working but no longer downgrade
...
* mailq: thresholds now take a duration (`1h`, `3D`) instead of a count. New `oldest_mail_age` perfdata. New `--mta` ([#781](https://github.com/Linuxfabrik/monitoring-plugins/issues/781))
* php-fpm-status: multi-pool via repeatable `--url`; HTTP basic auth in the URL. All perfdata labels renamed and prefixed `<pool>_` - update Grafana/InfluxDB queries
...
* gitlab-version: add `--check-security` to warn on security-relevant updates (default on) ([#688](https://github.com/Linuxfabrik/monitoring-plugins/issues/688))
...
* json-values: add `--token`/`--header` for HTTP auth; new `--warning-key`/`--critical-key` (dot-notation) for numeric alerts ([#1005](https://github.com/Linuxfabrik/monitoring-plugins/issues/1005))
* librenms-alerts, librenms-health: support device-type `management`
* logfile: `--filename` accepts time macros (`{today}`, `{%Y}`, ...) for date-stamped logs. Offset survives day rollovers ([#678](https://github.com/Linuxfabrik/monitoring-plugins/issues/678))
...
* scanrootkit: add 41 signatures for modern Linux rootkits and implants (BPFDoor, Drovorub, Ebury, FontOnLake, Kaiji, Kobalos, perfctl, PUMAKIT, Reptile, Symbiote, Winnti, and more)
* scanrootkit: add 11 further signatures for recent Linux threats (Auto-Color, Bootkitty, DslogdRAT, Hadooken, Koske, LinkPro, randkit, Snapekit, Sutekh, vmwfxs, WolfsBane)
* scanrootkit: findings now show the year the rootkit was first publicly disclosed
* scanrootkit: `rootkit_items` / `rootkit_possible` perfdata count distinct rootkits (was indicators); `max` field carries the signature-database size
...
* atlassian-statuspage: report primary incident, affected services and maintenance windows. New `--service REGEX`. Perfdata renamed (`impact` → `cnt_warn`/`cnt_crit`)
...
* logfile: output names the scanned file and each configured pattern with per-pattern match count and severity label ([#547](https://github.com/Linuxfabrik/monitoring-plugins/issues/547))
...
* redfish-sensor: `--insecure` now defaults to `True` (BMCs usually serve self-signed certs). Pass `--insecure=false` explicitly if a trusted CA chain is installed
* scanrootkit: exact per-symbol kernel symbol matching (fewer false positives)
* service: Windows services with a space in their technical name now match `--service` ([#921](https://github.com/Linuxfabrik/monitoring-plugins/issues/921))
...
* librenms-alerts: report `WORSE`/`BETTER`/`CHANGED` alerts too (was only `ACTIVE`); LibreNMS 25.2+ flips many to `CHANGED` ([#882](https://github.com/Linuxfabrik/monitoring-plugins/issues/882))
...
* redfish-drives: system-level warnings (e.g. inlet temp) no longer flip the check to WARN (covered by `redfish-system` now) ([#652](https://github.com/Linuxfabrik/monitoring-plugins/issues/652))
