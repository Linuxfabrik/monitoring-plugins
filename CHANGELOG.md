...
* mysql-innodb-buffer-pool-size: align with MySQLTuner v2.8.41 and ship Grafana dashboard. Absent or disabled InnoDB no longer triggers UNKNOWN. New check on `innodb_file_per_table` (mysqltuner alerts when it is OFF; per-table maintenance is much harder when everything lives in `ibdata1`). The log-size-ratio calc now uses `innodb_redo_log_capacity` on MySQL 8.0.30+ where it replaced `innodb_log_file_size * innodb_log_files_in_group` as the single sizing knob. The OK output now positively confirms `innodb_buffer_pool_size >= InnoDB data + index size` (mysqltuner emits a goodprint there too) instead of staying silent. When multiple checks fire, suggested actions are now collected under a single `Recommendations:` block at the end of the output instead of being interleaved with each finding. New perfdata: `mysql_innodb_data_size` (sum of data + index across all InnoDB tables) and `mysql_innodb_redo_log_capacity` (only when the variable exists). MySQL/MariaDB variable names in the output are wrapped in backticks for clarity. Grafana dashboard now also plots `innodb_log_files_in_group` next to the size series. Workload-based `innodb_redo_log_capacity` recommendation that mysqltuner derives from `Innodb_os_log_written` / uptime / RAM tiers is intentionally not implemented (would warrant its own plugin)
