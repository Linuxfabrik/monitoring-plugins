...
* logfile: `--filename` now accepts time macros (`{today}`, `{yesterday}`, `{%Y}`, `{%y}`, `{%m}`, `{%d}`, `{%H}`, `{%M}`, `{%S}`), so logfiles whose name contains the current date (Laravel `laravel-YYYY-MM-DD.log`, Windows `YYYYMMDD.log`, etc.) can be monitored without a wrapper script. Read offset and pending matches carry over when the filename changes on the next day ([#678](https://github.com/Linuxfabrik/monitoring-plugins/issues/678))
