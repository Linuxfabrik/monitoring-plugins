...
* mysql-aria: the absence or explicit deactivation of the Aria storage engine no longer triggers UNKNOWN. The plugin now reports OK with an info message in those cases (vanilla MySQL has no Aria, and disabling it is a config decision, not an unknown state). DESCRIPTION corrected to reflect that only the page-cache hit rate is tracked, not separate read and write rates. The two cumulative counters (previously emitted with the discouraged `uom='c'`) are now emitted as in-plugin-computed per-second rates (`mysql_aria_pagecache_read_requests_per_second`, `mysql_aria_pagecache_reads_per_second`); the deltas appear from the second check run onwards. Ships a dedicated Grafana dashboard with three panels splitting the metrics by magnitude (sizes, per-second read rates, hit-rate percentage) so the smaller counters no longer disappear next to the buffer-size series. When the hit rate is below 95% but the pagecache is already larger than the total Aria index size (so growing it further would not help), the OK output now explains why no warning is raised - mysqltuner alerts on the standalone hit rate, our intentional deviation is now visible to the admin reading the plugin output
* Director Basket: `mysql-binlog-cache` moved from the **MySQL Replication Service Set** to the baseline **MySQL Service Set**. Binary logging is enabled on standalone masters/sources too, not only on replicated setups, so the check belongs in the every-server baseline rather than under replication. Existing hosts that activated only the `mysql-replication` tag should also activate `mysql` to keep getting the binlog-cache check
* mysql-binlog-cache: `log_bin = OFF` no longer triggers UNKNOWN. Standalone servers without replication legitimately run with binary logging disabled, and the plugin now reports OK with an info message. Same for the freshly-started case where binary logging is on but `Binlog_cache_use = 0` (instead of misleadingly displaying "0% binlog cache memory access (0 memory / 0 total)"). When the cache has been used fewer than 10 times the output appends a "sample too small to act on" note so admins do not chase a hit rate built from a handful of transactions. The two cumulative counters (previously emitted with the discouraged `uom='c'`) are now emitted as in-plugin-computed per-second rates (`mysql_binlog_cache_use_per_second`, `mysql_binlog_cache_disk_use_per_second`); the deltas appear from the second check run onwards. DESCRIPTION rewritten in mysqltuner terminology ("memory-access rate"). Ships a dedicated Grafana dashboard with three panels splitting the metrics by magnitude
* mysql-joins: align with MySQLTuner v2.8.41 and ship Grafana dashboard. Recommendation now mirrors mysqltuner's nuance: only suggest raising `join_buffer_size` when it is still below 4 MiB; above that the buffer is already considered large enough and only "use JOINs with indexes" is suggested (raising further mostly hurts memory usage without helping). Output now always shows the count and the per-component breakdown (`Select_range_check`, `Select_full_join`) instead of just "Everything is ok." on the OK case. Three cumulative counters (`Select_full_join`, `Select_range_check`, derived `joins_without_indexes`) are now emitted as in-plugin-computed per-second rates instead of `uom='c'` continuous counters; the deltas appear from the second check run onwards. MySQL/MariaDB variable names wrapped in backticks. Grafana dashboard now also plots server uptime, so the per-second rates panel can be read against the server-restart timeline
* mysql-innodb-log-waits: align with MySQLTuner v2.8.41 and ship Grafana dashboard. Plugin scope broadened from a single check (Log Waits) to two related InnoDB log buffer health metrics that mysqltuner emits side by side: the existing **Log Waits** ratio (`Innodb_log_waits / Innodb_log_writes`, any wait > 0 alerts) plus the new **Write Log efficiency** ((`Innodb_log_write_requests - Innodb_log_writes`) / `Innodb_log_write_requests` * 100, alerts below 90%). Absent or disabled InnoDB no longer triggers UNKNOWN. Output now mirrors mysqltuner's percentage-based phrasing (`InnoDB log waits: 0.05% (X waits / Y writes)`). When both checks fire, the suggested action is now listed once under a single `Recommendations:` block at the end of the output instead of being repeated after each finding. The two cumulative counters and the new `Innodb_log_write_requests` are emitted as in-plugin-computed per-second rates instead of `uom='c'` continuous counters; the deltas appear from the second check run onwards. New perfdata: `mysql_innodb_log_waits_pct` and `mysql_innodb_write_log_efficiency_pct` (the latter only when the server exposes `Innodb_log_write_requests`). Edge case where `Innodb_log_writes > Innodb_log_write_requests` (physically impossible, can briefly appear during counter resets) is reported as an info note instead of triggering a phantom alert. Variable names wrapped in backticks
* mysql-innodb-buffer-pool-size!: align with MySQLTuner v2.8.41 and ship Grafana dashboard. Absent or disabled InnoDB no longer triggers UNKNOWN. New check on `innodb_file_per_table` (mysqltuner alerts when it is OFF; per-table maintenance is much harder when everything lives in `ibdata1`). The old 25%-log-size-ratio rule (`innodb_log_file_size * innodb_log_files_in_group` vs. `innodb_buffer_pool_size`, supposed band 20-30%) is replaced with the workload-based redo-log sizing path that mysqltuner v2.8.41 itself switched to: on MySQL 8.0.30+ the plugin now derives the recommended `innodb_redo_log_capacity` from the hourly `Innodb_os_log_written` write rate, rounded into the host's RAM tier (< 2 GiB / < 8 GiB / >= 8 GiB), and WARNs when the configured capacity is more than 10% below that target. On servers without `innodb_redo_log_capacity` (MariaDB, MySQL < 8.0.30) the redo-log sizing check is skipped with an info line; the `innodb_log_file_size` is still emitted as perfdata for trending. The OK output now positively confirms `innodb_buffer_pool_size >= InnoDB data + index size` (mysqltuner emits a goodprint there too) instead of staying silent. When multiple checks fire, suggested actions are now collected under a single `Recommendations:` block at the end of the output instead of being interleaved with each finding. Perfdata changes (admin-facing breaking change for custom Grafana panels): removed `mysql_innodb_log_files_in_group` and `mysql_innodb_log_size_pct`; added `mysql_innodb_data_size` (sum of data + index across all InnoDB tables), `mysql_innodb_redo_log_capacity` (only on MySQL 8.0.30+), `mysql_innodb_os_log_written_per_hour` and `mysql_innodb_redo_log_capacity_recommended` (both only on MySQL 8.0.30+ with at least 1 h uptime). MySQL/MariaDB variable names in the output are wrapped in backticks for clarity. Grafana dashboard panels now group by topic (sizes, redo log write rate) and replace the dropped log-size-ratio / log-files-in-group panels
* mysql-innodb-buffer-pool-instances: align with MySQLTuner v2.8.41 and ship Grafana dashboard. The check is now skipped (with an info message, state OK) on MariaDB 10.4+ where `innodb_buffer_pool_instances` was deprecated and later removed. Absent or disabled InnoDB no longer triggers UNKNOWN. The recommendation calculation now caps at the number of logical CPU cores on the host running the plugin (mysqltuner does the same), in addition to the 64 hard cap and the 1-instance-per-GiB rule. The output is consolidated into a single line that names the recommended value once and notes the CPU cap when it kicks in. Ships a dedicated Grafana dashboard with two panels (instances count and buffer pool size in bytes)
* mysql-database-metrics: new `--ignore-schemas REGEX` mirrors `--ignore-tables` but skips entire schemas (no aggregate contribution, no checks). New `--lengthy` adds a per-schema breakdown table to the plugin output, mirroring what `mysqltuner --dbstat` prints (Tables, Rows, Data, Index, Total, Engines, Table Collations, Column Charsets, Column Collations). The Director Basket now defaults to `--lengthy=true` and pre-fills `--ignore-schemas` with `^icinga` so admins running the shipped `MySQL Schemas Service Set` get the verbose table without the well-known Icinga-ecosystem mixed-collation noise (Icinga Director, Icinga Web 2 and Icinga DB ship schemas with mixed utf8/utf8mb4 by design)
* mysql-database-metrics: align with MySQLTuner v2.8.41 and ship perfdata. The `percona` schema is now excluded from aggregates and per-schema checks alongside `mysql`/`information_schema`/`performance_schema`/`sys` (a Percona Server Toolkit install used to be flagged falsely). The duplicate "multi storage engines"/"multi table engines" alerts are merged into a single "mixed storage engines" message (both were the same `count(distinct ENGINE)` query under different aliases). The `'NULL'` string compare in the index-vs-data-size guard is fixed to `is not None` (MySQL returns SQL NULL as Python `None`, never the string `'NULL'`, so the guard never triggered before). The plugin now emits five perfdata series for trending (`mysql_database_count`, `mysql_total_data_size`, `mysql_total_index_size`, `mysql_total_rows`, `mysql_total_tables`) and ships a dedicated Grafana dashboard. New optional `--ignore-tables REGEX` parameter excludes matching tables from every aggregate and per-schema check (useful for muting noisy temp/backup tables that legitimately differ from the schema-wide engine/collation). Empty schemas are now surfaced as an info note in the output instead of being silently skipped, and the OK output now leads with "Everything is ok." followed by the scanned scope (number of schemas, tables, rows, data and index size) instead of bare "Everything is ok."
* mysql-connections: name-resolution check broadened to mysqltuner's coverage. The plugin now suppresses the warning when `skip_networking=ON` (MySQL/MariaDB then accepts no TCP connections, so name resolution is moot) and when the MySQL/MariaDB version does not expose `skip_name_resolve` at all. Both the textual `ON`/`OFF` and the numeric `1`/`0` forms that some versions return are accepted. When multiple checks fire, suggested actions are now collected under a single `Recommendations:` block at the end of the output instead of being repeated per finding. The two cumulative counters (`Aborted_connects`, `Connections`, previously emitted with the discouraged `uom='c'`) are now emitted as in-plugin-computed per-second rates (`mysql_aborted_connects_per_second`, `mysql_connections_per_second`); the deltas appear from the second check run onwards. New perfdata `mysql_pct_max_connections_used` graphs the lifetime peak ratio that mysqltuner alerts on. The Grafana dashboard now has four panels (counts, per-second rates, percentages, timeouts) instead of three, with the per-second rates panel replacing the old `non_negative_difference()` workaround on the cumulative counter
* mysql-logfile: refresh against MySQLTuner v2.8.41 with output cleanup. On MySQL 8.0.22+ the plugin now prefers `performance_schema.error_log` (works remote, no shell access to the log file needed); falls back transparently to the on-disk file, container or systemd source as before. Bug fix: `docker:`/`podman:`/`kubectl:` sources used to silently fall through into the file-path branch and exit WARN with "not an existing regular file" - the container path now actually reads the logs. Severity detection switched from substring (`error in line`, `warning in line`) to mysqltuner's bracket-tag match (`[ERROR]`/`[Warning]`); table or schema names containing the words "error" or "warning" no longer trip the counts. OK output now positively confirms what was checked ("No errors found. No warnings found. 1 startup detected. ..."). When several findings apply, suggested actions are collected under a single `Recommendations:` block at the end of the output. DESCRIPTION clarified about when root/sudo is actually required. Unit tests added (static log fixtures covering OK, warning, error, restart-history and bracket-tag-only cases plus ignore-pattern/ignore-regex/always-ok)
* mysql-memory: refresh against MySQLTuner v2.8.41 with multiple admin-visible changes. Bug fix: `max_tmp_table_size` is now `min(tmp_table_size, max_heap_table_size)` (the previous `max()` over-estimated `server_buffers`; the effective in-memory temp table size is physically capped at the minimum). Galera GCache memory is now counted toward `max_used_memory` / `max_peak_memory` when `wsrep_on=ON` and `wsrep_provider_options` contains a `gcache.size` (typical 128 MiB - 1 GiB on cluster nodes). Performance Schema memory and other-process memory are now emitted as perfdata (`mysql_pf_memory`, `mysql_gcache_memory`, `mysql_other_process_memory`). New `--warning` (default 85%) and `--critical` (default 95%) parameters control the threshold at which the reached or peak footprint flips state; the previously-hardcoded 95% was both unusable for tuning and inconsistent with mysqltuner's 85% cutoff. Output reformatted: short summary by default ("Max used memory: X% (Y of Z physical). Peak possible: A% (B). Other process memory: C."). Whenever a memory-related WARN/CRIT fires, the `Recommendations:` block also lists the top 5 contributors to peak memory in descending order so the admin immediately sees which knob dominates (per-thread buffers like `max_allowed_packet` or `sort_buffer_size` often outrank `innodb_buffer_pool_size` once `max_connections` is large). Full ranked breakdown of every contributor behind `--lengthy`. MySQL/MariaDB variable names wrapped in backticks. Recommendations consolidated into a single `Recommendations:` block at the end. Performance Schema OFF is now surfaced as an info line in the output. Grafana dashboard rounded out so all collected metrics are actually plotted (`max_allowed_packet`, `query_cache_size`, `server_buffers`, `per_thread_buffers`, `total_per_thread_buffers`, `max_total_per_thread_buffers`, `pf_memory`, `gcache_memory`, `other_process_memory`). Other-process memory filter now matches by process `name` instead of `cmdline` substring (no longer accidentally excludes `mysqld-exporter` and similar). psutil < 5.3.0 fallback code path removed (RHEL8 ships psutil 5.4+)
* mysql-open-files: align with MySQLTuner v2.8.41 wording. New `--warning` (default 85%) / `--critical` (default 95%) parameters replace the previously hard-coded 85% threshold and add a CRIT escalation. MySQL/MariaDB variable names in the output are wrapped in backticks. WARN message moves the "raise `open_files_limit`" suggestion into a `Recommendations:` block. DESCRIPTION clarifies that `Open_files` is server-wide and `open_files_limit` is per-process (file-descriptor ceiling)
* mysql-query!: align with the wording / structure of the other mysql-* plugins. Perfdata renamed (admin-facing breaking change for custom Grafana panels): `cnt_warn` -> `mysql_query_warn_value`, `cnt_crit` -> `mysql_query_crit_value`. Output reformatted: "WARN query (`<sql>`) returned <value> [STATE]" / "CRIT query (`<sql>`) returned <value> [STATE]" instead of "X results from warning query ...". Long queries (>80 chars) are now ellipsised in the summary line so the output stays one readable row; the full query still runs and is graded. OK output leads with "Everything is ok." followed by the per-query summaries. Truncation magic numbers (`10`, `5`, `5`) extracted to named constants
* mysql-perf-metrics: refresh against MySQLTuner v2.8.41. The duplicate `innodb_file_per_table` check is removed - it lives in `mysql-innodb-buffer-pool-size` (admins with both checks no longer see the same warning twice). DESCRIPTION explains the two remaining knobs (`innodb_stats_on_metadata`, `concurrent_insert`) in admin terms instead of mysqltuner-perl jargon. OK output now leads with "Everything is ok." followed by the verified values ("`innodb_stats_on_metadata` is `OFF`. `concurrent_insert` is `AUTO`.") so an admin sees both the verdict and what was checked. WARN output collects suggested actions into a single `Recommendations:` block at the end. MySQL/MariaDB variable names wrapped in backticks. Typo fixed: "Stat are updated" -> "`innodb_stats_on_metadata` is `ON`" with a clearer recommendation. Both knobs now emit numeric perfdata (`mysql_innodb_stats_on_metadata`: 0=OFF / 1=ON, `mysql_concurrent_insert`: 0=NEVER / 1=AUTO / 2=ALWAYS) so config drift is trendable and Icinga colours the perfdata band correctly. Ships a dedicated Grafana dashboard with two panels for the encoded values
* mysql-open-files, mysql-slow-queries, mysql-sorts, mysql-table-cache, mysql-table-definition-cache, mysql-table-locks, mysql-temp-tables, mysql-thread-cache, mysql-traffic: ship Grafana dashboards. Panels group the existing perfdata by unit/topic (sizes, lifetime counters, percentages, rates) so admins no longer have to build dashboards by hand for these checks
* mysql-slow-queries: README clarifies that `Slow_queries` is a status counter that increments regardless of whether `slow_query_log` is on. The log only controls whether the offending queries are persisted to a file - the counter the plugin reads is independent
* mysql-slow-queries!: refresh against MySQLTuner v2.8.41. Bug fix: `Slow_queries` / `Questions` ratio was compared as `int(...) > 5`, so a ratio of 5.9% rounded down to 5 and never triggered. Now compared as a float against the new `--warning` / `--critical` thresholds. Bug fix: when `Questions = 0` (a freshly booted, idle server) the plugin used to mutate `mystat['Questions']` to 1 before computing the ratio - that fake value then leaked into the perfdata as `mysql_questions = 1`. The division is now guarded locally and the perfdata reports the real value. New `--warning` (default 5%) / `--critical` (default 10%) parameters control the WARN/CRIT thresholds; the previous hard-coded 5% lived in the source. MySQL/MariaDB variable names wrapped in backticks. Suggested actions move into a single `Recommendations:` block at the end of the output (`long_query_time` > 10s nudge and "Enable `slow_query_log`" hint). OK output leads with "Everything is ok." followed by the verified values. Perfdata renamed (admin-facing breaking change for custom Grafana panels): the cumulative `mysql_questions` and `mysql_slow_queries` counters are replaced by per-second rates `mysql_questions_per_second` and `mysql_slow_queries_per_second` (in-plugin-computed against a local SQLite cache). Grafana dashboard updated accordingly
* mysql-table-locks!: refresh against MySQLTuner v2.8.41. New `--warning` (default 95%) / `--critical` (default 85%) replace the hardcoded 95% WARN-only threshold. The WARN default matches mysqltuner; the CRIT escalation is new so the plugin state goes red on severe MyISAM/Aria lock contention. Output reformatted: OK leads with "Everything is ok." followed by the verified percentage and the immediate/total counts; WARN/CRIT findings carry `[WARNING]` / `[CRITICAL]` markers and collect the suggested action into a single `Recommendations:` block at the end. The recommendation now spells out the underlying mechanic (table-level locks vs. `InnoDB` row-level locks) so the admin knows why migrating to `InnoDB` helps. Perfdata renamed (admin-facing breaking change for custom Grafana panels): the cumulative `mysql_table_locks_immediate` and `mysql_table_locks_waited` counters are replaced by per-second rates `mysql_table_locks_immediate_per_second` and `mysql_table_locks_waited_per_second` (in-plugin-computed against a local SQLite cache). `mysql_pct_table_locks_immediate` is now pinned to `100.0` on contention-free servers (`Table_locks_waited = 0`) instead of returning `0` when neither counter is set yet. MySQL/MariaDB variable names in the output are wrapped in backticks
* mysql-table-indexes!: refresh against MySQLTuner v2.8.41 plus a new replication-relevant check. Performance: the previous implementation issued one `SHOW INDEX` per user table; on a server with 100 schemas and 10000 tables this was 10000+ round-trips. Replaced with two single-shot queries against `information_schema` (a `LEFT JOIN` for "no index at all" and a `NOT EXISTS` for "InnoDB without `PRIMARY KEY`"). New `--warning` (default 1) / `--critical` (default 10) replace the implicit "any bad table = WARN" behaviour. New `--ignore-schemas REGEX` and `--ignore-tables REGEX` regex filters mirror the ones in `mysql-database-metrics` / `mysql-storage-engines`. New `--lengthy` prints the full list of affected tables; without it the output truncates at 10 per category to keep the Icinga service line manageable. New replication-relevant check: InnoDB base tables without a user-defined `PRIMARY KEY` are now reported separately from the generic "no index at all" finding; missing `PRIMARY KEY` on InnoDB is a documented performance hotspot for ROW-based replication (the replica falls back to a full table scan per row event). DESCRIPTION fixed: the previous text claimed the plugin checks for "primary keys" and "duplicate indexes", neither of which the code actually verified. Output reformatted: OK leads with "Everything is ok." followed by the total base-table count; WARN/CRIT findings carry `[WARNING]` / `[CRITICAL]` markers and collect the suggested action into a single `Recommendations:` block at the end of the output. Schema and table names in the output are wrapped in backticks. New perfdata: `mysql_total_tables`, `mysql_tables_without_index`, `mysql_innodb_tables_without_primary_key` (plugin previously had no perfdata). Director Basket service template now defaults to `--lengthy=true` and `enable_perfdata: true` (was `false`). Ships a dedicated Grafana dashboard with one panel that plots the three counts against each other
* mysql-table-definition-cache: refresh against MySQLTuner v2.8.41. Output reformatted: OK now leads with "Everything is ok." followed by the verified `table_definition_cache` value and the current `information_schema.tables` count (was previously bare "Everything is ok." with no scope). WARN now uses the canonical `[WARNING]` state marker and moves the "raise `table_definition_cache` above N, or set it to `-1` (autosizing)" suggestion into a single `Recommendations:` block at the end of the output. English fix: "is lower than" / "is upper than" replaced with "is less than" / "is greater than" (matches mysqltuner's `badprint` / `goodprint` wording). Variable names in the output are wrapped in backticks. The autosizing sentinel `-1` is now encoded as `0` in perfdata so the Grafana scale stays meaningful (a negative spike would dominate); the OK message still names the actual setting
* mysql-memory, mysql-open-files, mysql-slow-queries, mysql-sorts, mysql-storage-engines, mysql-table-cache, mysql-table-indexes, mysql-table-locks: `--warning` and `--critical` now accept Nagios range expressions (`80`, `10:`, `~:50`, `@10:20`) instead of plain numbers, matching the convention documented in [THRESHOLDS.md](https://github.com/Linuxfabrik/monitoring-plugins/blob/main/THRESHOLDS.md). The shipped defaults already use the new syntax (`'85'`, `'95:'`, ...). Setups that previously passed a plain number like `--warning 25` keep working unchanged; the new syntax also unlocks asymmetric ranges and inverted (`@`) thresholds. Boundary semantics shift slightly: `--warning 5` previously alerted at `value >= 5`, now alerts at `value > 5`. For integer-count thresholds (`mysql-table-indexes`, fragmented tables in `mysql-storage-engines`) the defaults were adjusted from `1`/`5`/`10` to `0`/`4`/`9` to preserve the "any defect = WARN" / "five+ = CRIT" semantics under Nagios range rules
* mysql-temp-tables!: new `--warning` / `--critical` (Nagios range syntax) replace the hardcoded 25%-on-disk WARN-only threshold; CRIT default at 50%. Bug fix: the plugin previously crashed (`KeyError`) on a freshly booted server that had not yet materialised a temporary table. Perfdata: cumulative `mysql_created_tmp_disk_tables` / `mysql_created_tmp_tables` replaced by per-second rates (breaking change for custom Grafana panels). Output now follows the canonical facts-first / `Recommendations:` layout used by the other mysql-* plugins
* mysql-traffic!: refresh against MySQLTuner v2.8.41. Bug fix: a server with no traffic yet (no SELECTs and no writes) used to report "100% writes" because the read-ratio math fell through to a `0%/100%` else branch; both percentages now stay at `0%` in that case (matches mysqltuner). Bug fix: the message line read `Uptime` straight from the raw status value, ignoring the 1-second clamp the queries-per-second math uses, so on the first run after a restart the message showed "Up 0s (4.0 qps)" - both now share the clamped value. Perfdata renamed (admin-facing breaking change for custom Grafana panels): the cumulative counters `mysql_bytes_received`, `mysql_bytes_sent`, `mysql_com_*`, `mysql_connections` and `mysql_questions` are replaced by per-second rates `*_per_second` (in-plugin-computed against a local SQLite cache); the deltas appear from the second check run onwards. `Bytes_received_per_second` / `Bytes_sent_per_second` now carry the `B` unit. TX/RX figures in the message line use `bytes2human` instead of `number2human` so they read as `560.2MiB` rather than `560.2M`
* mysql-thread-cache!: refresh against MySQLTuner v2.8.41. New `--warning` (default `50:`) / `--critical` (default `30:`) Nagios-range thresholds replace the hardcoded 50% WARN-only cutoff. Bug fix: `mysql_thread_cache_size` perfdata was tagged as `B` (Bytes); it is a count of threads. Perfdata: cumulative `mysql_connections` / `mysql_threads_created` replaced by per-second rates (breaking change for custom Grafana panels); new `mysql_threads_cached` shows current cache utilisation. Output reformatted to the canonical facts-first / `Recommendations:` layout used by the other refreshed mysql-* plugins. README clarifies that the cache warm-up suppression (skip alert during the first hour of uptime) is a Linuxfabrik addition on top of mysqltuner's behaviour
* mysql-table-cache!: refresh against MySQLTuner v2.8.41. New `--warning` (default 20%) / `--critical` (default 10%) replace the hardcoded 20% WARN floor; the WARN default matches mysqltuner, the CRIT escalation is new so the plugin state goes red before the cache is essentially useless. Output reformatted: OK leads with "Everything is ok." followed by the verified hit rate, miss count and `table_open_cache` / `open_files_limit` values; WARN/CRIT findings carry `[WARNING]` / `[CRITICAL]` markers and collect the suggested action into a single `Recommendations:` block at the end (raise `table_open_cache` gradually, keep `open_files_limit` above it, and the MyISAM-only scalability caveat that mysqltuner cites). Perfdata renamed (admin-facing breaking change for custom Grafana panels): the cumulative `mysql_table_open_cache_hits` and `mysql_table_open_cache_misses` counters are replaced by per-second rates `mysql_table_open_cache_hits_per_second` and `mysql_table_open_cache_misses_per_second` (in-plugin-computed against a local SQLite cache). New `Important Note` in the README about transient WARN/CRIT after a cold restart (the cache is empty, every initial table open is a miss)
* mysql-storage-engines!: refresh against MySQLTuner v2.8.41. The `AUTO_INCREMENT` check now compares each column to its own type ceiling (`TINYINT`/`SMALLINT`/`MEDIUMINT`/`INT`/`BIGINT`, signed/unsigned) instead of always against `BIGINT UNSIGNED` like mysqltuner does - on the most common modern column type (`INT UNSIGNED`, 4-byte) a table about to hit the 4.3-billion ceiling would never have reached 1% of `BIGINT UNSIGNED` and the old check was effectively dead code. Output reformatted: facts-first line shows total table/engine counts and the highest `AUTO_INCREMENT` percentage; suggested SQL (`ALTER TABLE ... FORCE` for InnoDB, `OPTIMIZE TABLE` for the rest, matching mysqltuner's per-engine rule) moves into a single `Recommendations:` block at the end of the output. New `--lengthy` flag emits a per-engine breakdown table (Engine, Tables, Total, Data, Index). New `--warning-fragmented-tables` / `--critical-fragmented-tables` (defaults 1 / 5) and `--warning-autoincrement-pct` / `--critical-autoincrement-pct` (defaults 75 / 90) replace the hardcoded thresholds. New `--ignore-schemas` and `--ignore-tables` regex filters mirror the ones in `mysql-database-metrics`. Schema/table names in the output are wrapped in backticks. Removed the dead-code `have_bdb` and `have_isam` branches (the variables were removed from MySQL around 2008 and are absent on every supported server). The mysqltuner advice "Add skip-innodb to MySQL configuration" is replaced with a more useful pointer ("investigate why no `InnoDB` tables exist"); the `skip-innodb` option itself was removed in MySQL 5.7 / MariaDB 10.1. New perfdata: `mysql_autoincrement_max_pct`, `mysql_fragmented_data_free`, `mysql_fragmented_tables`, `mysql_table_count`, `mysql_total_size`. Director Basket service template now defaults to `--lengthy=true` and `enable_perfdata: true`. Ships a dedicated Grafana dashboard with three panels (table counts, sizes, `AUTO_INCREMENT` usage)
* mysql-sorts!: refresh against MySQLTuner v2.8.41. New `--warning` (default 10%) / `--critical` (default 20%) parameters control the WARN/CRIT thresholds; the previous hard-coded 10% cut-off was in the source. MySQL/MariaDB variable names wrapped in backticks. Suggested action moves into a single `Recommendations:` block at the end of the output. OK output leads with "Everything is ok." followed by the verified values. Output rewording: "Sorts requiring a temporary merge-sort file" instead of "Sorts requiring temporary tables" (sort spills go to merge-sort files, not in-memory temp tables; the old wording mixed concepts up). Perfdata renamed (admin-facing breaking change for custom Grafana panels): the cumulative `mysql_sort_scan`, `mysql_sort_range`, `mysql_sort_merge_passes` and `mysql_total_sorts` counters are replaced by per-second rates `mysql_sort_scan_per_second`, `mysql_sort_range_per_second`, `mysql_sort_merge_passes_per_second` and `mysql_total_sorts_per_second` (in-plugin-computed against a local SQLite cache). Grafana dashboard updated accordingly
* mysql-replica-status!: refresh against MySQLTuner v2.8.41. Bug fix: replication-lag detection was firing on every server because the plugin treated each missing `Seconds_Behind_Master` / `Seconds_Behind_Source` as a lag indicator - on MariaDB 10.5+ only `Seconds_Behind_Source` exists and on older versions only `Seconds_Behind_Master`, so the absent field always triggered a false-positive WARN "lagging behind primary". The plugin now uses defined-or semantics and picks whichever field the server actually exposes. Documented privilege requirement narrowed from `SUPER, REPLICATION CLIENT, REPLICATION SLAVE` to the actually needed `SLAVE MONITOR` (or `REPLICA MONITOR` on MariaDB 11+). On MariaDB 10.5+, plain `REPLICATION CLIENT` alone no longer covers `SHOW REPLICA STATUS` / `SHOW SLAVE STATUS` (MariaDB split the privilege). New `--lag-warning` (default 0, matches mysqltuner's "any lag" cutoff) and `--lag-critical` (no default; lag never escalates to CRIT unless set) parameters control the lag threshold; the existing `--severity` keeps controlling the threshold-less alerts (IO/SQL thread not running, `read_only` disabled). New parallel-replication checks on MariaDB 10.5+: alerts when `replica_parallel_mode` / `slave_parallel_mode` is not `optimistic` and reminds the admin that `binlog_format` must be `ROW` on the primary for parallel replication. Reports parallel-replication thread count in the output. Output reformatted: short summary uses `. `-joined facts with MySQL/MariaDB variable names in backticks; OK output leads with "Everything is ok."; suggested actions move into a single `Recommendations:` block at the end. New perfdata: `mysql_replication_seconds_behind` (lag in seconds, only when replicating), `mysql_replication_io_running` (0/1), `mysql_replication_sql_running` (0/1), `mysql_replication_slave_count` (downstream replicas this server is feeding). Ships a dedicated Grafana dashboard with three panels (lag, IO/SQL thread state, downstream replica count). Typo `DEFAULT_SERVERITY` -> `DEFAULT_SEVERITY` fixed
* mysql-system: refresh against MySQLTuner v2.8.41. New WARN on `fs.nr_open < 1M` (matches mysqltuner's check). Listening-port count is now correctly limited to TCP ports in `LISTEN` state - the previous implementation counted all TCP connections (`LISTEN` + `ESTABLISHED` + `TIME_WAIT` …) and could over-trigger `--maxportsallowed` by orders of magnitude on busy hosts. WARN messages name the source of the limit (`--maxportsallowed`) and explain that the count is system-wide. Recommendation strings now mention both the runtime `echo X > /proc/sys/...` form and the persistent `/etc/sysctl.conf` form, matching mysqltuner. When several settings need attention, the recommendations are collected under a single `Recommendations:` block at the end of the output instead of being mixed into each finding sentence. The OK output enumerates every verified setting instead of just "Everything is ok.". Perfdata labels were normalised to underscores (admin-facing breaking change for custom Grafana panels): `mysql_kernel_vm.swappiness` → `mysql_kernel_vm_swappiness`, `mysql_kernel_sunrpc.tcp_slot_table_entries` → `mysql_kernel_sunrpc_tcp_slot_table_entries`, `mysql_kernel_fs.aio-max-nr` → `mysql_kernel_fs_aio_max_nr`, `mysql_opened_ports` → `mysql_listening_ports`. The shipped Grafana panel was updated; an `fs.nr_open` series was added to the file-handles panel
...
* mysql-user-security: refresh against MySQLTuner v2.8.41. On MySQL 5.7+ servers with the `validate_password` plugin active the username-as-password check is now skipped (mysqltuner-aligned workaround for [MySQL Bug #80860](https://bugs.mysql.com/bug.php?id=80860); previously the comparison failed at SQL parse time and the finding was silently absent without explanation). MariaDB roles (`mysql.user.IS_ROLE = 'Y'`) are now excluded from the anonymous-user and username-as-password checks so a role with an empty user name no longer counts as an anonymous account. Suggested SQL statements now use a literal `<replace-with-strong-password>` placeholder instead of a randomly-generated 40-character password (the random one risked accidentally going live via a copy-paste from the monitoring log). Output now enumerates the scope (total non-system user accounts and roles) plus a one-line summary per check, even on OK ("0 anonymous accounts. 0 users without password. ..."), so the admin sees what was actually verified. New perfdata: `mysql_user_count`, `mysql_role_count`, `mysql_anonymous_users`, `mysql_users_without_password`, `mysql_users_with_username_as_password`, `mysql_users_with_wildcard_host`. Director Basket: `enable_perfdata` flipped to `true` (was `false`). Typo fix: internal `DEFAULT_SERVERITY` -> `DEFAULT_SEVERITY`
* Grafana dashboards: panel lines stay continuous instead of breaking into disconnected dots when Grafana's auto-bucketed `$interval` is finer than the actual Icinga check interval (typical case at 2-day or 7-day zoom levels with default 60-second checks). InfluxDB queries gained `fill(previous)` so empty buckets inherit the last known value. `spanNulls: false` is unchanged, so real check outages still show as a clean break in the line
* Icinga Web Grafana module: drop the stale `[cmd-check-<plugin>-windows]` and `[cmd-check-<plugin>-windows-python]` sections from the per-plugin `icingaweb2-module-grafana/*.ini` files of plugins that have no `.windows` marker (i.e. that are not shipped for Windows). Affects `apache-httpd-status`, `keycloak-memory-usage`, `mysql-connections`, `mysql-logfile`, `mysql-memory`, `mysql-system`. The removed sections pointed at `<plugin> - Windows` Grafana dashboards that don't exist; the cleanup is purely cosmetic (Icinga ignored the orphaned sections silently)
...
* logfile: `--filename` now accepts time macros (`{today}`, `{yesterday}`, `{%Y}`, `{%y}`, `{%m}`, `{%d}`, `{%H}`, `{%M}`, `{%S}`), so logfiles whose name contains the current date (`20260422.log`, `app-2026-04-22.log`, etc.) can be monitored without a wrapper script. `{today}` / `{yesterday}` resolve tolerantly: the compact form (`YYYYMMDD`) is tried first, ISO 8601 (`YYYY-MM-DD`) is used as fallback. Read offset and pending matches carry over when the filename changes on the next day ([#678](https://github.com/Linuxfabrik/monitoring-plugins/issues/678))
