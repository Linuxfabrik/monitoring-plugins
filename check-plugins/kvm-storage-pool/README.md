...
* **The name of a store is the deepest path holding every pool on it, so it moves when the set of pools does.** Four pools under `/home` and `/var` of one root filesystem are reported as `/`; stop the two under `/home`, or exclude them with `--ignore`, and the same store is reported as `/var`. The metrics are named after it, so a dashboard sees the old series end and a new one begin. Anything that has to survive that, an alert rule or a saved panel query, is better matched on the pool name in the table than on the metric name.
