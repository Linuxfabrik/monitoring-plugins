...
|----|----|----|
| nginx_connections_accepted_per_second | Number | Accepted client connections per second. |
| nginx_connections_active | Number | The current number of active client connections including waiting connections. |
| nginx_connections_handled_per_second | Number | Handled connections per second. Generally matches the accepted rate unless some resource limits have been reached (for example, the `worker_connections` limit). |
...
| nginx_http_requests_per_second | Number | Client requests per second. |
