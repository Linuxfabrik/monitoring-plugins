...
The plugin ships ready-to-use Containerfiles for every supported distribution under [`unit-test/containerfiles/`](https://github.com/Linuxfabrik/monitoring-plugins/tree/main/check-plugins/ups-nut/unit-test/containerfiles) (`archlinux-vlatest`, `debian-v12/v13`, `fedora-v43`, `rhel-v8/v9/v10`, `sles-v15/v16`, `ubuntu-v2204/v2404/v2604`). Each one installs `nut-server` with a dummy UPS profile, exposes upsd on TCP 3493 and configures an `admin` user with the password `linuxfabrik` for testing the auth path.
