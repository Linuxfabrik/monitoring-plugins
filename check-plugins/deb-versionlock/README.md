...
* The shipped Basic Service Sets turn `--check-pinning` on and ignore `influxdata-archive-keyring`, a package pinned to a negative priority on purpose because it would drop a second, unmanaged repository file on the host. Drop that pattern from the service where the site wants to see it, and add your own with `--ignore` for every pin that is deliberate on your hosts.
