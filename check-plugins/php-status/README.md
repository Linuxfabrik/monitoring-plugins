...
A persistent misconception is that uploading a large file needs a correspondingly large `memory_limit`. It does not. PHP streams a multipart file upload **straight to a temporary file on disk** (`upload_tmp_dir`) in small chunks, so the file never accumulates in the script's memory, which is what `memory_limit` bounds. You can run `upload_max_filesize = 2G` with `memory_limit = 128M`. What an upload needs is **disk** space in `upload_tmp_dir`, not RAM.

`post_max_size` is narrower than it looks as well. PHP only reads and parses a request body when the request method is `POST`; a `PUT` never touches the directive. That is how a chunked WebDAV upload works, which is what Nextcloud and comparable applications use, and why such a host can accept a 10 GB file with `post_max_size = 16M`. Even for a POST, only `$_POST` and `$_FILES` are affected: an application reading the raw body from `php://input`, a JSON API for example, still receives it in full.

Where the two directives do meet is the classic HTML form upload (`multipart/form-data`). The file travels inside the POST body, so PHP measures the whole body against `post_max_size` before `upload_max_filesize` is ever reached, and rejects the request with empty `$_POST` and `$_FILES`. On such a host `upload_max_filesize` is only reachable when `post_max_size` is larger than it. The surcharge is a constant handful of bytes rather than a percentage, roughly 200 bytes per file plus the length of the file name, plus whatever other fields the form submits, so a full MiB on top is generous and costs nothing: `upload_max_filesize = 64M` with `post_max_size = 65M`. A form that accepts several files at once sends all of them in one POST, so there `post_max_size` has to cover the number of files times `upload_max_filesize`, with `max_file_uploads` (default 20) capping that number.

The web server in front of PHP has the last word regardless of the method. It measures every request body against its own limit and answers `413 Request Entity Too Large` by itself, in which case none of the php.ini values come into play. Apache httpd caps the body with `LimitRequestBody`, nginx with `client_max_body_size`. Neither is unlimited when the directive is absent: httpd then falls back to 1 GiB and nginx to 1 MiB, and only an explicit `0` lifts the limit. Hardened configurations set httpd much lower, 100 KiB being a common value, which breaks a chunked upload just as thoroughly as a form upload. The plugin does not see any of these, so check them along with the php.ini values. `httpd -t -D DUMP_INCLUDES` lists the files an Apache configuration pulls in, ready to grep for `LimitRequestBody`; `nginx -T` prints the whole nginx configuration the same way. The apache-httpd-security and nginx-security checks report both directives.
