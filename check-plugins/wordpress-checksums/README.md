...
* Old releases cannot be verified, see [Supported WordPress Releases](#supported-wordpress-releases) below. The core is covered from 3.6 in English and from 3.7 at the earliest in other languages. Below that, wordpress.org publishes nothing to compare against, and the check says so instead of reporting a clean result.
...
```text
WordPress v6.8.2 (en_US): 1 modified [CRITICAL]. 2834 files verified in 3 of 3 components.
...
wordpress.org publishes nothing for this release, so there is nothing to hold the installation to and the rest of the result says nothing about the core. Either the release predates the checksum archive, which starts at 3.6 for English installations and later for the others (see Supported WordPress Releases above), or the version in `wp-includes/version.php` is not one wordpress.org ever released. `wordpress-version` says which of the two it is. Where the release is simply too old, updating is the fix; there is nothing to configure here.
