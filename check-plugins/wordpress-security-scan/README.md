...
* **How many sites a plan carries.** Every component the scan detects costs one lookup, and on the free plan 25 of them are granted per day. A typical site spends four (two plugins, one theme, the core), so a free plan carries roughly six sites at the recommended one scan per day. Past that the allowance runs out, and every further scan that day reports `Your API limit has been reached`, drops the token and runs on without vulnerability data. `--enterprise-db-token` removes the ceiling entirely, because a locally held copy answers the lookups without asking anyone.
* **An exhausted allowance is noticed before the scan, not during it.** The check asks the API what is left of the daily allowance before it starts, which is a free question the allowance itself does not pay for. Where nothing is left it scans without the token straight away and names the hour the allowance returns. This buys the answer rather than the time: the scanner gives up before it enumerates anything, so the attempt that fails costs seconds and not a second scan. What it says on its own is only that the limit has been reached.
* **A refused lookup is not the same as one nobody asked for.** A run that was never given a token was set up that way and follows `--no-vuln-data-severity` (default `ok`). A token that was rejected or rotated, an exhausted daily allowance and an unreachable API follow `--vuln-api-failure-severity` (default `warn`) instead: the scan carries on and still reports everything else, so without its own state nothing would say that the vulnerability class has stopped being examined. Set it to `ok` for the old behaviour.
* **How wide the scan looks depends on what a vulnerability lookup costs.** Against the WPScan API every component the scan detects costs one request out of a daily allowance, and the free plan grants 25 of them per day: a site with a few dozen plugins exhausts a whole day in a single run. `--enterprise-db-token` answers the same lookups from a copy held on the host, where they cost nothing. So with the API the check keeps the default `vp,vt`, and with a local copy it hands `wpscan` the union of its popular and its vulnerable slugs through `--plugins-list` / `--themes-list` instead, which is everything the scanner knows about. The run says which of the two it did, and `--wpscan-enumerate` overrides the decision, because an explicit choice is an instruction rather than a default to improve upon.
* The reason the wider scan needs a slug list at all is that `--enumerate` cannot express it. `vp` and `p` name two different lists, the narrower one wins for probing, and `vp` additionally switches on an output filter that drops everything not already vulnerable, so asking for both costs the wide scan and returns the narrow result. Neither list contains the other: of the plugins `wpscan` knows, 1'498 are flagged popular and 7'343 vulnerable, with only 886 in both. That is why a scan restricted to `vp` misses a plugin that is merely out of date, and one restricted to `p` misses a vulnerable plugin nobody installs much.
* Finding out that a component is out of date costs no API request at all. `wpscan` compares the detected version against the `latest_version` in its local metadata, so the outdated findings appear even on a run whose token was rejected. Only the vulnerability lookup is paid for.
...
usage: wordpress-security-scan [-h] [-V] [--always-ok] [--api-token API_TOKEN]
                               ...
                               [-v]
                               [--vuln-api-failure-severity {ok,warn,crit,unknown}]
...
options:
  ...
  -v, --verbose         Makes this plugin verbose during the operation. Useful
                        ...
                        hood.
  --vuln-api-failure-severity {ok,warn,crit,unknown}
                        State to report when the vulnerability database
                        refused the request or could not be reached: a token
                        that was rejected or rotated, an exhausted daily
                        request allowance, or an unreachable API. The scan
                        itself still runs and everything it found still
                        counts, but the vulnerability class is no longer being
                        examined and nothing else says so. A run that was
                        never given a token follows `--no-vuln-data-severity`
                        instead, because that one was set up that way and is
                        not a failure. Default: warn
  --wpscan-detection-mode {aggressive,mixed,passive}
...
* Per `--no-vuln-data-severity` (default `ok`) if no API token was configured, and per `--vuln-api-failure-severity` (default `warn`) if one was and the lookup was refused or could not be reached: a rejected or rotated token, an exhausted daily allowance, an unreachable database. The scan itself still runs and reports everything it found. The summary then reads "Vulnerabilities not checked" rather than "No vulnerabilities found", the reason is named in the output, and `vuln_data_available=0` marks the run in the perfdata. Everything the scan could determine without that data still counts towards the state.
