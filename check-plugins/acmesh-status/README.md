...
* The check needs to read the acme.sh configuration directory. acme.sh creates it as `drwx------` owned by root, so the check runs with sudo. A host that has not deployed the sudoers file makes the service report UNKNOWN until it has.
