...
Checks the hit rate for open table cache lookups in MySQL/MariaDB. A low hit rate indicates that `table_open_cache` may need to be increased. Logic is taken from [MySQLTuner](https://github.com/jmrenouard/MySQLTuner-perl):mysql_stats() and has been verified in sync with MySQLTuner v2.8.41.
