...
* `--check-excludes` additionally reads `exclude` / `excludepkgs` from the main configuration and from every `.repo` file in the directories `reposdir` names. Two configurations take an exclusion out of force on dnf 4 without removing it from the file: a repository whose section is `enabled=0`, which the package manager skips whole, and `disable_excludes`, which switches exclusion off for `all` scopes, for `main`, or for a named repository. dnf 5 knows neither the option nor a command line switch for it and applies the exclusions regardless. Such an entry is therefore still reported, and the summary says how many of them dnf 4 leaves alone, so the same files produce the same list on both generations. The main configuration is one file: `/etc/dnf/dnf.conf`, or `/etc/yum.conf` where that is the only one present. Where `/etc/yum.conf` survives as a file of its own beside the dnf one, the package manager reads neither it nor its exclusions, and neither does the check. Where it names none, all four default directories are searched: `/etc/yum.repos.d`, `/etc/yum/repos.d`, `/etc/distro.repos.d` and `/usr/share/dnf5/repos.d`. The last two generations disagree on one entry each, dnf 4 not reading the dnf 5 directory and dnf 5 not reading `/etc/yum/repos.d`, so an exclusion left in the directory belonging to the other generation is reported although nothing applies it. A `main` section inside a `.repo` file is skipped, since the package manager reads its main configuration from one file only.
...
### `dnf 4 leaves N of the exclusions out of force`

The exclusions are in the files, but on dnf 4 something switches them off: the repository section they sit in is `enabled=0`, or `disable_excludes` in `/etc/dnf/dnf.conf` names `all`, `main`, or that repository. dnf 5 ignores `disable_excludes` altogether and applies them anyway, which is why the entries are reported on both generations instead of disappearing on one of them.
