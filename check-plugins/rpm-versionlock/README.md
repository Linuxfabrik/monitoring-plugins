...
* The shipped Basic Service Sets turn `--check-excludes` on and exclude the specs that vendor repository files carry as a matter of course: `*beta*` (Grafana's own repository file keeps pre-releases out), `grafana-enterprise*` (a mirror that syncs the community packages only), and `influxdata-archive-keyring` (a package that would drop a second, unmanaged repository file on the host). Drop those patterns from the service where the site wants to see them, and add your own with `--ignore` for every exclusion that is deliberate on your hosts.
